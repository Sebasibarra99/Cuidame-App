...
import com.sebastian.ecovibra1.databinding.ActivityCambiarPasswordBinding
...
class CambiarPassword : AppCompatActivity() {
    ...
    private fun validarInformacion() {
        ...
        if(pass_actual.isEmpty()){
            binding.etPassActual.error = "Ingrese contrasena actual"
            binding.etPassActual.requestFocus()
        }
        else if(pass_nueva.isEmpty()){
            binding.etPassNueva.error = "Ingrese nueva contrasena"
            binding.etPassNueva.requestFocus()
        }
        else if(r_pass_nueva.isEmpty()){
            binding.etRPassNueva.error = "Repita nueva contrasena"
            binding.etRPassNueva.requestFocus()
        }
        else if(pass_nueva != r_pass_nueva){
            binding.etRPassNueva.error = "No coinciden las contrasenas"
            binding.etRPassNueva.requestFocus()
    ...
    private fun actualizarPassword() {
        progressDialog.setMessage("Cambiando contrasena")
        progressDialog.show()
        ...
        firebaseUser.updatePassword(pass_nueva)
            .addOnSuccessListener {
                ...
                Toast.makeText(
                    this,
                    "La contrasena se ha actualizado",
                    Toast.LENGTH_SHORT
                ).show()
                cerrarSesion()
